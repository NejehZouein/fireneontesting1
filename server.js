    ...
    switch (comando) {
      ...
      case prefix + "help":
        message.channel.send(
          "📜 Lista de comandos:\n" +
            "```xl\n" +
            "'fm!play'          Reproducir una canción o añadirla a la cola.\n" +
            "'fm!pause'         Pausar la canción actual.\n" +
            "'fm!resume'        Resumir la canción pausada.\n" +
            "'fm!queue'         Ver lista de canciones que están en cola de reproducción.\n" +
            "'fm!skip'          Saltar canción que se está reproduciendo.\n" +
            "'fm!disconnect'    Sacar el bot del canal de voz.\n" +
            "'fm!servers'       Mostrar la cantidad de servidores que ha sido invitado el bot.\n" +
            "'fm!help'          Lista de comandos." +
            "```"
        );
        break;
...
client.on("ready", function() {
  ...
  client.user.setPresence({
    game: {
      name: "|| Usa ft!help", // Estado del bot
      type: 0
    }
  });
});
...
function reproduciendo(id, url, titulo, duracion, message) {
  console.log("ID: " + id);
  message.channel.send(
    "🔊 Reproduciendo la siguiente canción:```fix\n🎵: " +
      titulo +
      "\n⏲️: [" +
      duracion +
      "]\n📽️: " +
